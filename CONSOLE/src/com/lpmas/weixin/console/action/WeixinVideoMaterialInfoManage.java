...
/**
 * Servlet implementation class WeixinVideoMaterialInfoManage
 */
@WebServlet("/weixin/WeixinVideoMaterialInfoManage.do")
public class WeixinVideoMaterialInfoManage extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Logger log = LoggerFactory.getLogger(WeixinVideoMaterialInfoManage.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public WeixinVideoMaterialInfoManage() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 获取请求参数
		String videoType = ParamKit.getParameter(request, "voiceType", "").trim();
		String videoData = ParamKit.getParameter(request, "voiceData", "").trim();
		if (!StringKit.isValid(videoType) || !StringKit.isValid(videoData)) {
		...
		// 保存临时图片
		byte[] voice = BASE64.decodeBase64(videoData);
		...
		try {
			...
			addMaterialRequestBean.setMediaType(MaterialTypeConfig.MT_VIDEO);
			...
			if (StringKit.isValid(bean.getMediaId())) {
				bean.set_id(
						WeixinMongoConfig.getMaterilInfoKey(accountId, MaterialTypeConfig.MT_VIDEO, bean.getMediaId()));
				bean.setCreateTime((int) (System.currentTimeMillis() / 1000));
				bean.setAccountId(accountId);
				bean.setMediaType(MaterialTypeConfig.MT_VIDEO);
