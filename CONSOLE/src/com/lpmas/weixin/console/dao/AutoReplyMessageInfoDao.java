...
public class AutoReplyMessageInfoDao {
	...
	public PageResultBean<AutoReplyMessageInfoBean> getAutoReplyMessageInfoPageListByMap(
		...
		try {
			...
			String replyName = condMap.get("replyName");
			if (StringKit.isValid(replyName)) {
				condList.add("reply_name = ?");
				paramList.add(replyName);
			}
	...
	public List<AutoReplyMessageInfoBean> getAutoReplyMessageInfoListByMap(HashMap<String, String> condMap) {
		List<AutoReplyMessageInfoBean> list = new ArrayList<AutoReplyMessageInfoBean>();
		DBFactory dbFactory = new WeixinDBFactory();
		DBObject db = null;
		try {
			db = dbFactory.getDBObjectR();
			...
			DBExecutor dbExecutor = dbFactory.getDBExecutor();
			list = dbExecutor.getRecordListResult(sql, orderQuery, condList, paramList, AutoReplyMessageInfoBean.class,
					db);
		} catch (Exception e) {
			log.error("", e);
		} finally {
