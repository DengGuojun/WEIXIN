<%@page import="com.lpmas.weixin.api.config.ReceiveMessageTypeConfig"%>
...
<%
    ...
    List<ReceiveMessageBean> list =(List<ReceiveMessageBean>)request.getAttribute("ReceiveMessageList");
...
<body class="article_bg">
	<p class="article_tit">微信公众号消息列表</p>
	<form action="WeixinReceiveMessageList.do" method="post" >
		<div class="search_form">
			  ...
			  <select name="accountId" id="accountId">
				...
				<em class="int_label">消息类型：</em>
			  <select name="msgType" id="msgType">
			  <option value=""></option>
				<%String msgType = ParamKit.getParameter(request, "msgType", "").trim();
				for(StatusBean<String,String> statusBean:ReceiveMessageTypeConfig.RECEIVE_MESSAGE_TYPE_LIST){%>
				    <option value="<%=statusBean.getStatus()%>" <%=msgType.equals(statusBean.getStatus()) ? "selected" : "" %>><%=statusBean.getValue()%></option>
				<%}%>
				</select>
    ...
    <%
    for(ReceiveMessageBean bean:list){
    %> 
    <tr>
      <td><%=MapKit.getValueFromMap(bean.getAccountId(), accountMap) %></td>
      <td><%=ReceiveMessageTypeConfig.RECEIVE_MESSAGE_TYPE_MAP.get(bean.getMsgType()) %></td>
      <td><%=bean.getCreateTime()>0? DateKit.formatDate(new Date(bean.getCreateTime()), DateKit.DEFAULT_DATE_TIME_FORMAT):"--"%></td>
      <td align="center">
      	<a href="WeixinReceiveMessageManage.do?_id=<%=bean.get_id().toHexString()%>&accountId=<%=bean.getAccountId()%>">查看</a>
      </td>
    </tr>	
    <%} %>
  </table>
